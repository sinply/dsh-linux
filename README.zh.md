...
- [构建指南 docs/build.md](docs/build.md) —— 如何从 dsh 源码重新构建本发行包（一键：`powershell -File scripts\build-linux.ps1`）
- [版本更新说明 CHANGELOG.zh.md](CHANGELOG.zh.md) —— 每个版本的上游变更、破坏性变更与升级注意（[English](CHANGELOG.md)）
...
├── docs/                  # 使用 + 构建文档、operator README 源文件
├── scripts/build-linux.ps1   # 一键打包（Windows + WSL）
├── scripts/linux-assemble/   # 可复现构建脚本 + build-all.sh 驱动
